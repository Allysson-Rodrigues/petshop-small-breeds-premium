...
def check_page(file_path: Path) -> dict:
    ...
    # 1. Title tag
    has_title = ('<title' in content.lower() or
                 'title=' in content.lower() or
                 '<Head>' in content or
                 'SEO:' in content.upper() or
                 'title:' in content.lower())
    if not has_title and is_layout:
        issues.append("Missing <title> tag")

    # 2. Meta description
    has_description = ('name="description"' in content.lower() or
                       'name=\'description\'' in content.lower() or
                       'content="description"' in content.lower() or
                       'description:' in content.lower())
    if not has_description and is_layout:
        issues.append("Missing meta description")

    # 3. Open Graph tags
    has_og = ('og:' in content.lower() or
              'property="og:' in content.lower() or
              'property=\'og:' in content.lower() or
              'property="og' in content.lower())
