...
class UXAuditor:
    ...
    def audit_file(self, filepath: str) -> None:
        ...
        # Use a more specific regex to avoid matching inside comments, script tags, or strings that just contain the word
        ...
        # Check for color names (more strict regex)
        purple_names = ['purple', 'violet', 'fuchsia', 'magenta', 'lavender']
        for name in purple_names:
            if re.search(r'\b' + name + r'\b', content_no_comments, re.IGNORECASE):
                # Check if it's likely a CSS value or Tailwind class
                if re.search(r'(?:color|bg|border|fill|stroke|text)-' + name + r'|' + name + r'\s*;', content_no_comments, re.IGNORECASE):
                    self.issues.append(f"[Color] {filename}: PURPLE DETECTED ('{name}'). Banned by Maestro rules. Use Teal/Cyan/Emerald instead.")
                    break
    ...
    def audit_directory(self, directory: str) -> None:
        ...
        for root, dirs, files in os.walk(directory):
            dirs[:] = [d for d in dirs if d not in {'node_modules', '.git', 'dist', 'build', '.next', 'archive', '02-playground', '03-estudos', '.agent', 'biblioteca-conhecimento', 'dotfiles-repo'}]
            for file in files:
